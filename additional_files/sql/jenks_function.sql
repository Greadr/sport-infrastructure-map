...
CREATE OR REPLACE FUNCTION geo.CDB_JenksBins(in_array NUMERIC[], breaks INT, iterations INT DEFAULT 0, invert BOOLEAN DEFAULT FALSE)
RETURNS NUMERIC[] as
$$
DECLARE
    in_matrix NUMERIC[][];
...
BEGIN
    ...
    best_result = geo.CDB_JenksBinsIteration(in_matrix, breaks, classes, invert, sdam, shuffles);
    ...
    LOOP
        ...
        curr_result = geo.CDB_JenksBinsIteration(in_matrix, breaks, classes, invert, sdam, shuffles);
...
DROP FUNCTION IF EXISTS geo.CDB_JenksBinsIteration ( in_matrix NUMERIC[], breaks INT, classes INT[], invert BOOLEAN, element_count INT4, arr_mean NUMERIC, max_search INT); -- Old signature

CREATE OR REPLACE FUNCTION geo.CDB_JenksBinsIteration ( in_matrix NUMERIC[], breaks INT, classes INT[], invert BOOLEAN, sdam NUMERIC, max_search INT DEFAULT 50) RETURNS NUMERIC[] as $$
DECLARE
    i INT;
    iterations INT = 0;
